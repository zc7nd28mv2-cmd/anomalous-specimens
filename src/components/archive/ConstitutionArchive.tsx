...
import { constitutionFor } from "@/lib/constitution";
...
export function ConstitutionArchive() {
  ...
  const data = constitutionFor(activeSpecimenId);
  const specimen = getSpecimen(activeSpecimenId);
  const materialLines = [
    data.groups[0].items.join(" / "),
    data.groups[1].items.join(" / "),
    data.groups[2].items.join(" / "),
  ] as [string, string, string];
  const [topStream, setTopStream] = useState<string>(data.traces["01"]);
  const [heartStream, setHeartStream] = useState<string>(data.traces["02"]);
  const [baseStream, setBaseStream] = useState<string>(data.traces["03"]);
  ...
  return (
    <div className="relative min-h-dvh overflow-x-hidden bg-bg px-5 py-16 sm:px-10 sm:py-20 md:px-16">
      <div className="story-content mx-auto md:ml-[6vw]">
        <h1 className="title-module text-ink">{data.title}</h1>
        <p className="aux-en mt-2">{data.en}</p>
        <p className="sys-meta mt-6">{data.record}</p>

        <p className="compose-code mt-12">{data.scan}</p>

        <div className="mt-10 space-y-10">
          <NoteBlock
            title={`【${data.groups[0].zh}】`}
            trace={topStream}
            running={analyzing}
            showPlate={showPlate}
            typed={phase === "stable" ? materialLines[0] : typed[0]}
            result={data.result}
          />
          <NoteBlock
            title={`【${data.groups[1].zh}】`}
            trace={heartStream}
            running={analyzing}
            showPlate={showPlate}
            typed={phase === "stable" ? materialLines[1] : typed[1]}
            result={data.result}
          />
          <NoteBlock
            title={`【${data.groups[2].zh}】`}
            trace={baseStream}
            running={analyzing}
            showPlate={showPlate}
            typed={phase === "stable" ? materialLines[2] : typed[2]}
            result={data.result}
          />
        </div>

        <BackLink label={`返回 ${specimen.name}`} onClick={() => go("specimen")} />
      </div>
    </div>
  );
}
...
function NoteBlock({
  ...
  return (
    <section>
      ...
      <p className="compose-result mt-5">{result}</p>
